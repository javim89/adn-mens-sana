  ...
  return (
    <div>
      ...
      <p className="text-[#6B7280] mb-6">
        Citaciones de plantel para los próximos partidos
        {total > 0 && (
          <span className="ml-2 text-xs font-medium bg-gray-100 text-[#6B7280] px-2 py-0.5 rounded-full">
            {total} {total === 1 ? 'convocatoria' : 'convocatorias'}
          </span>
        )}
      </p>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        <CustomSelect
          ...
          onChange={(v) => handleCategoriaChange(v === '__all__' ? '' : v)}
          searchable
            ...
            items.map((c) => (
              <li key={c.id} className="px-4 py-4">
                <div className="flex items-start justify-between gap-2">
                  <Link href={`/convocatorias/${c.id}`} className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[#1C1C1C]">
                      {formatDate(c.fecha)}
                    </p>
                    <p className="text-xs font-medium text-[#1C1C1C] mt-0.5">
                      {c.rival}
                    </p>
                    <p className="text-xs text-[#6B7280] mt-0.5">
                      {c.disciplinaNombre} · {c.categoriaNombre}
                    </p>
                    <p className="text-xs text-[#6B7280] mt-1">
                      {c.totalConvocados}{' '}
                      {c.totalConvocados === 1 ? 'convocado' : 'convocados'}
                    </p>
                  </Link>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <Link
                      href={`/convocatorias/${c.id}/editar`}
                      className="text-[#6B7280] hover:text-[#121A61] transition-colors p-1"
                    ...
                    <DeleteConvocatoriaModal
                      convocatoriaId={c.id}
                      convocatoriaLabel={`${formatDate(c.fecha)} — ${c.disciplinaNombre}`}
          ...
          <table className="w-full">
            <thead>
              <tr className="text-xs uppercase text-[#6B7280] bg-[#F3F4F6] border-b border-gray-100">
                <th className="px-5 py-3 text-left">Fecha del partido</th>
                <th className="px-5 py-3 text-left">Partido</th>
                <th className="px-5 py-3 text-left">Disciplina</th>
                <th className="px-5 py-3 text-left">Categoría</th>
                <th className="px-5 py-3 text-left">Convocados</th>
              ...
              {items.length === 0 ? (
                ...
                items.map((c) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-sm text-[#1C1C1C]">
                      {c.rival}
                    </td>
                    <td className="px-5 py-3.5 text-sm text-[#6B7280]">
                      {c.disciplinaNombre}
                    </td>
                    <td className="px-5 py-3.5 text-sm text-[#6B7280]">{c.categoriaNombre}</td>
