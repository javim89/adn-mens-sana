...
interface Props {
  ...
  onChange: (selected: DeportistaOption[]) => void;
}
...
export default function DeportistaMultiSelect({ value, onChange }: Props) {
  ...
  function toggle(dep: DeportistaOption) {
    if (selectedIds.has(dep.id)) {
      onChange(value.filter((v) => v.id !== dep.id));
    } else {
      onChange([...value, dep]);
    }
  }
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        <span className="text-[#6B7280]">
          {value.length === 0
            ? 'Buscar y seleccionar deportistas...'
          ...
          <ul className="max-h-52 overflow-y-auto">
            {isLoading ? (
              ...
              <li className="px-3 py-6 text-center text-sm text-[#6B7280]">
                {options.length === 0 ? 'No se encontraron deportistas' : 'Todos ya seleccionados'}
              </li>
            ) : (
              availableOptions.map((dep) => (
                <li key={dep.id}>
                  <button
                    type="button"
                    onClick={() => toggle(dep)}
